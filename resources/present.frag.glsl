...
layout(binding = 3, rgba32f) uniform image2D source_distance;
...
vec3 hq4x_color(vec4 tex_coords[4], vec2 texture_size) {
...
vec3 hq4x_depth(vec4 tex_coords[4], vec2 texture_size) {
	vec2 ps = 1.0 / texture_size;
	...
	vec3 p1  = vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[0].xy  * texture_size)).x);
	vec3 p2  = vec3(0, 0, imageLoad(source_distance, ivec2((tex_coords[0].xy + vec2(dx, dy) * quad)  * texture_size)).x);
	vec3 p3  = vec3(0, 0, imageLoad(source_distance, ivec2((tex_coords[0].xy + vec2(dx, 0) * quad)  * texture_size)).x);
	vec3 p4  = vec3(0, 0, imageLoad(source_distance, ivec2((tex_coords[0].xy + vec2(0, dy) * quad)  * texture_size)).x);
	mat4x3 pixels = mat4x3(p1, p2, p3, p4);

	vec3 w1  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[1].xw * texture_size)).x);
	vec3 w2  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[1].yw * texture_size)).x);
	vec3 w3  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[1].zw * texture_size)).x);

	vec3 w4  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[2].xw * texture_size)).x);
	vec3 w5  = yuv * p1;
	vec3 w6  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[2].zw * texture_size)).x);

	vec3 w7  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[3].xw * texture_size)).x);
	vec3 w8  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[3].yw * texture_size)).x);
	vec3 w9  = yuv * vec3(0, 0, imageLoad(source_distance, ivec2(tex_coords[3].zw * texture_size)).x);
...
void main() {
	...
	vec3 res_color = hq4x_color(tex_coords, texture_size);
	float res_depth = hq4x_depth(tex_coords, texture_size).x;

	out_final = vec4(res_color, 1.0);
	gl_FragDepth = res_depth;
}
